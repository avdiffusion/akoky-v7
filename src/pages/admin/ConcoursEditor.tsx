...
const emptyTranslation = (): ConcoursTranslation => ({
  slug: "",
  metaTitle: "",
  metaDescription: "",
  title: "",
...
const ConcoursEditor = () => {
  ...
  const [article, setArticle] = useState<ConcoursArticle>({
    id: generateConcoursId(),
    image: "",
  ...
  const [activeLang, setActiveLang] = useState<ConcoursLang>("fr");
  ...
  useEffect(() => {
    ...
    if (!isNew && id) {
      const existing = getConcoursById(id);
      if (existing) {
        setArticle(existing);
      } else {
        toast.error("Concours introuvable");
        navigate("/admin/concours");
      }
    }
  }, [id, isNew, navigate]);

  const getTranslation = (lang: ConcoursLang): ConcoursTranslation => {
    return article.translations[lang] || emptyTranslation();
  };

  const updateTranslation = (lang: ConcoursLang, updates: Partial<ConcoursTranslation>) => {
    setArticle((prev) => ({
      ...prev,
      translations: {
        ...prev.translations,
  ...
  const handleTitleChange = (lang: ConcoursLang, title: string) => {
    const t = getTranslation(lang);
    const updates: Partial<ConcoursTranslation> = { title };
  ...
  const handleSave = () => {
    ...
    saveConcours(article);
    toast.success(isNew ? "Concours créé !" : "Concours mis à jour !");
    navigate("/admin/concours");
  };

  const publishAll = () => {
    const updated = { ...article };
    CONCOURS_LANGS.forEach((l) => {
  ...
  const publishedCount = CONCOURS_LANGS.filter((l) => article.translations[l]?.published).length;
      ...
      <header className="sticky top-0 z-50 bg-card/80 backdrop-blur border-b border-border">
        <div className="container max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate("/admin/concours")}>
              <ArrowLeft className="h-4 w-4 mr-1" /> Retour
            </Button>
            <h1 className="text-lg font-bold text-foreground font-display">
              {isNew ? "Nouveau concours" : "Modifier le concours"}
            </h1>
          </div>
      ...
      <div className="container max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-card border border-border rounded-xl p-4 space-y-3">
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <Input
                    type="date"
                    value={article.startDate.slice(0, 10)}
                    onChange={(e) => setArticle((prev) => ({ ...prev, startDate: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="date"
                    value={article.endDate.slice(0, 10)}
                    onChange={(e) => setArticle((prev) => ({ ...prev, endDate: e.target.value }))}
                  />
                </div>
              </div>
            </div>
            ...
            {/* Prize */}
            <div className="bg-card border border-border rounded-xl p-4 space-y-3">
              <label className="text-sm font-medium text-foreground">🏆 Lot principal</label>
              <Input
                value={article.prize}
                onChange={(e) => setArticle((prev) => ({ ...prev, prize: e.target.value }))}
                placeholder="Ex: Séjour Cap d'Agde + 4500€"
              />
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5">
                        {hasContent && <Badge variant="secondary" className="text-[10px]">✓</Badge>}
                        {tr?.published ? (
          ...
          <div className="lg:col-span-3">
            <Tabs value={activeLang} onValueChange={(v) => setActiveLang(v as ConcoursLang)}>
                ...
                <TabsContent key={lang} value={lang} className="space-y-6">
                  {preview ? (
                    <div className="bg-card border border-border rounded-2xl p-8">
                      ...
                      {article.image && (
                        <img src={article.image} alt="" className="w-full rounded-xl object-cover aspect-video mb-6" />
                      )}
                      <div
                        className="prose prose-invert max-w-none"
                        dangerouslySetInnerHTML={{ __html: getTranslation(lang).content || "<p>Contenu…</p>" }}
                      />
                    </div>
                  ) : (
                    <>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-foreground">Titre ({LANG_LABELS[lang]})</label>
                        <Input
                          value={getTranslation(lang).title}
                          onChange={(e) => handleTitleChange(lang, e.target.value)}
                          placeholder={`Titre du concours en ${LANG_LABELS[lang]}`}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-card/50 border border-border rounded-xl p-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            placeholder="mon-concours-seo"
                          />
                          <p className="text-[10px] text-muted-foreground/60">
                            URL : /{lang}/concours/{getTranslation(lang).slug || "slug"}
                          </p>
                        </div>
                        <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between bg-card/50 border border-border rounded-xl p-4">
                        <div>
                          <p className="text-sm font-medium text-foreground">Publication ({LANG_LABELS[lang]})</p>
                          <p className="text-xs text-muted-foreground">
                            {getTranslation(lang).published ? "Ce concours est visible publiquement" : "Brouillon — non visible"}
                          </p>
                        </div>
                        <Button
                          variant={getTranslation(lang).published ? "default" : "outline"}
                          size="sm"
                          onClick={() => updateTranslation(lang, { published: !getTranslation(lang).published })}
                          className={getTranslation(lang).published ? "bg-green-600 hover:bg-green-700 text-white" : ""}
                        >
